...
ModelPurpose = Literal["decision", "toolcall", "default", "local", "gemini"]
...
class LLMManager:
    ...
    def get_llm_by_purpose(
        ...
        base_url = (os.getenv(api_config_override.get("base_url_env", ""))
                    or api_config_override.get("base_url")
                    or os.getenv(api_config.get("base_url_env", ""))
                    or api_config.get("base_url", _DEFAULT_BASE_URL))
        max_retries = api_config_override.get("max_retries") or api_config.get("max_retries", _DEFAULT_MAX_RETRIES)

        if "api_key_env" in api_config_override:
            api_key = os.getenv(api_config_override["api_key_env"], "")
        else:
            api_key = (api_config_override.get("api_key")
                       or os.getenv(api_config.get("api_key_env", ""))
