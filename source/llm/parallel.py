...
def invoke_llm_parallel_with_func(
    ...
    # Default concurrency for model-serving endpoints
    ...
    try:
        # Submit all tasks. We snapshot the current contextvars context (which
        # carries the LLM caller / phase tags set by @graph_node) and run each
        # task inside a fresh copy. Without this, ContextVar values set in the
        # main thread (e.g. _current_caller) do NOT propagate into worker
        # threads, so all LLM stats end up bucketed under the 'unknown' caller.
        parent_ctx = contextvars.copy_context()
        future_to_index = {
            executor.submit(parent_ctx.copy().run, func, task): idx
