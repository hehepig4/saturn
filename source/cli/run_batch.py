...
Usage:
    ...
    python source/cli/run_batch.py

    # Run specific datasets
    python source/cli/run_batch.py --datasets "fetaqa public_bi"

    # Skip unify stage (data already in unified format)
    python source/cli/run_batch.py --skip-unify

    # Custom Stage 1 parameters
    python source/cli/run_batch.py --stage1-total-queries 200 --stage1-llm gemini

    # Dry run - show what would be executed
    python source/cli/run_batch.py --dry-run
"""
...
Examples:
  ...
  python source/cli/run_batch.py --skip-unify --skip-ingest
  ...
  python source/cli/run_batch.py --force --datasets fetaqa
        """
    )
    ...
    for d in datasets:
        ...
        if args.rag_type is not None:
            config.stage1_rag_type = args.rag_type
            config.query_analysis_rag_type = args.rag_type
            config.layer2_rag_type = args.rag_type
    ...
    if args.db_path:
        ...
        # Symlink global ontology tables (TBox) so skip-stage1 Stage 2+ can read TBox
        for ont_tbl in ["ontology_metadata", "ontology_classes", "ontology_properties"]:
            src = main_db / f"{ont_tbl}.lance"
            dst = db_path / f"{ont_tbl}.lance"
            if src.exists() and not dst.exists():
                os.symlink(src, dst)
                logger.debug(f"  Symlinked global: {ont_tbl}.lance")
    ...
    # Store this run's logs and usage statistics together.
    reuse_tag = "no_reuse" if args.disable_reuse else "with_reuse"
    exp_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    exp_dir = PROJECT_ROOT / "logs" / "runs" / f"{reuse_tag}_{exp_ts}"
