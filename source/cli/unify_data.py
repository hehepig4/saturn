...
Supported datasets:
    ...
    - openwikitable: Open-WikiTable chunks and original query splits
...
Usage:
    ...
    python source/cli/unify_data.py --all

    # Process single dataset with default config
    python source/cli/unify_data.py --dataset bird

    # Process single dataset with overrides
    python source/cli/unify_data.py --dataset fetaqapn --train-queries 2000 --translate

    # Use custom JSON config array: [[name, train_queries, translate], ...]
    python source/cli/unify_data.py --config '[["bird", 500, false], ["fetaqapn", 2000, true]]'
...
# Dataset type: 'pneuma', 'solo', or 'owt'
DATASET_CONFIGS = {
    ...
    "openwikitable": {"type": "owt"},  # OpenWikiTable (splitted chunks)
}
...
# ============ OpenWikiTable Format Processing ============

def process_owt_tables(
    raw_dir: Path,
    dataset_name: str,
    output_dir: Path
) -> Dict[int, str]:
    """
    Process OpenWikiTable splitted_tables.json into unified JSON format.

    Returns mapping from chunk_id (int) -> unified table_id string.
    """
    output_dir.mkdir(parents=True, exist_ok=True)

    splitted_file = raw_dir / "splitted_tables.json"
    logger.info(f"Loading splitted_tables from {splitted_file} ...")
    with open(splitted_file, encoding="utf-8") as f:
        raw = json.load(f)
    st_df = pd.DataFrame.from_dict(raw)
    logger.info(f"Loaded {len(st_df)} table chunks.")

    chunk_id_to_table_id: Dict[int, str] = {}

    for _, row in tqdm(st_df.iterrows(), total=len(st_df), desc="Processing OWT tables"):
        chunk_id = int(row["id"])
        table_id = f"owt-{chunk_id}"
        if chunk_id in chunk_id_to_table_id:
            raise ValueError(f"Duplicate OpenWikiTable chunk ID: {chunk_id}")
        ...
        table_data = {
            ...
            "source_file": "splitted_tables.json",
            "columns": header,
            ...
            "metadata": {
                "dataset": dataset_name,
                "original_table_id": row.get("original_table_id") or "",
                "chunk_id": chunk_id,
                "table_name": row.get("name") or "",
                "caption": row.get("caption") or "",
                "source_dataset": row.get("dataset") or "",
            },
        }

        output_file = output_dir / f"owt-{chunk_id}.json"
        with open(output_file, "w", encoding="utf-8") as f:
            json.dump(table_data, f, ensure_ascii=False, indent=2)

        chunk_id_to_table_id[chunk_id] = table_id

    logger.info(f"Processed {len(chunk_id_to_table_id)} OWT table chunks.")
    return chunk_id_to_table_id


def process_owt_queries(
    raw_dir: Path,
    dataset_name: str,
    output_dir: Path,
    chunk_id_to_table_id: Dict[int, str]
    ...
    """
    Process OWT QA splits (train/valid/test) into unified JSONL format.

    Preserve the source train/valid/test splits. The conceptualization query
    budget is applied by the pipeline, not by truncating the source split.
    Returns (train_count, valid_count, test_count).
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    for split in ["train", "valid", "test"]:
        qa_df = pd.read_json(raw_dir / f"{split}.json")
        out_file = output_dir / f"{split}.jsonl"
        n = 0
        with open(out_file, "w", encoding="utf-8") as fout:
            for _, row in tqdm(qa_df.iterrows(), total=len(qa_df), desc=f"OWT {split} queries"):
                hard_pos = list(row["hard_positive_idx"]) if row["hard_positive_idx"] else []
                pos = list(row["positive_idx"]) if row["positive_idx"] else []
                # Build answer_tables: union of hard_positive + positive, in that order, deduplicated
                seen: set = set()
                answer_tables: List[str] = []
                for idx in hard_pos + pos:
                    tid = chunk_id_to_table_id.get(int(idx))
                    if tid is None:
                        raise ValueError(f"Unknown chunk {idx} in {split} query {row['question_id']}")
                    if tid not in seen:
                        seen.add(tid)
                        answer_tables.append(tid)
                if not answer_tables:
                    raise ValueError(f"No answer tables for {split} query {row['question_id']}")

                answer_val = row["answer"]
                if isinstance(answer_val, list):
                    answer_str = ", ".join(str(a) for a in answer_val)
                else:
                ...
                unified = {
                    ...
                    "metadata": {
                        "dataset": dataset_name,
                        "source": "openwikitable",
                        "original_table_id": row["original_table_id"],
                        "sql": row.get("sql") or "",
                        "original_dataset": row.get("dataset") or "",
                        "hard_positive_idx": hard_pos,
                        "positive_idx": pos,
                        "negative_idx": list(row["negative_idx"]) if row["negative_idx"] else [],
        ...
        counts[split] = n
        logger.info(f"OWT {split}: {n} queries written to {out_file}")
...
def process_dataset(
    ...
    if config["type"] == "pneuma":
    ...
    elif config["type"] == "owt":
        # Process OpenWikiTable format
        raw_dir = BENCHMARK_RAW_DIR / dataset_name

        if not raw_dir.exists():
            raise FileNotFoundError(f"OWT raw data not found: {raw_dir}")

        # Process table chunks
        chunk_map = process_owt_tables(
            raw_dir,
            dataset_name,
            output_dir / "table"
        )
        stats["table_count"] = len(chunk_map)

        # Process QA splits
        train_count, valid_count, test_count = process_owt_queries(
            raw_dir,
            dataset_name,
            output_dir / "query",
            chunk_map
        ...
        stats["train_queries"] = train_count
        stats["valid_queries"] = valid_count
        stats["test_queries"] = test_count
