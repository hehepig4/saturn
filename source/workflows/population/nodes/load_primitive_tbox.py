...
def _load_from_lancedb(dataset_name: str, iteration: int = -1) -> tuple[List[str], List[Dict[str, Any]], Dict, Dict, str, List[Dict]]:
    ...
    Supports both "primitive_tbox" and "federated_primitive_tbox" types.
    ...
    # Prefer the most recent federated UPO snapshot.
    ...
    if not ontology:
        # A primitive_tbox has no iteration field.
    ...
    if not ontology:
        raise RuntimeError(
            ...
            f"python source/cli/run_pipeline.py --step federated_primitive_tbox"
        )
