...
- Query annotations: class and value annotations
...
class ColumnAnalysis(BaseModel):
    ...
    - column_type → TBox constraint for class-aware query representation
...
class UnifiedQueryAnalysis(BaseModel):
    ...
    - columns[].column_type/value → class-aware query representation
    """
...
The system retrieves tables by matching their profiles to a UPO-aligned description of the query.

Your job is to generate:
- A hypothetical table description
- Column descriptions aligned with the available column types, including specific values when the query provides them
...
def get_data_property_specs(class_names: List[str], dataset_name: str) -> str:
    ...
    try:
        ...
        try:
            metadata = store.db.open_table('ontology_metadata').to_pandas()
            ontologies = metadata[
                (metadata['dataset_name'] == dataset_name)
                & (metadata['ontology_type'] == 'federated_primitive_tbox')
            ].sort_values('created_at', ascending=False)
            if ontologies.empty:
                raise LookupError(f'No UPO found for {dataset_name}')
            ontology_id = ontologies.iloc[0]['ontology_id']
            dp_df = store.db.open_table('ontology_properties').to_pandas()
            dp_df = dp_df[dp_df['ontology_id'] == ontology_id]
...
def process_unified_analysis(
    ...
    """
    Process unified output for UPO-aligned retrieval.

    The reasoning field is reused:
    - For HyDE: reasoning serves as the hypothetical column description
    - Column types and values are retained as query annotations
    ...
    Returns:
        ...
        - constraint_set: ConstraintSet (class and value annotations)
...
async def _unified_query_analysis_impl(
    ...
    Returns:
        ...
        - constraint_set: ConstraintSet for query inspection
