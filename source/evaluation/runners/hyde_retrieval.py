...
Usage:
    ...
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --compare-table-desc

    # Compare raw query vs HyDE column descriptions
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --compare-column-desc

    # Full comparison of all modes
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --full-compare

    # Compare with specific retriever type
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --retriever bm25 --full-compare

    # Comprehensive evaluation: all retriever types × all HyDE modes
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --comprehensive

    # Analyze specific cases
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --case 1 2 3

    # Show failure cases
    python source/evaluation/runners/hyde_retrieval.py -d fetaqa --show-failures 5
"""
...
def load_unified_analysis(
    ...
    raise FileNotFoundError(
        ...
        f"Run: python cli/retrieval.py --analyze-queries -d {dataset} -n -1 --use-rag"
    )
...
def load_indexes_and_embedder(dataset: str):
    ...
    if not available_indexes:
        raise RuntimeError(
            ...
            f"Run: python source/cli/run_pipeline.py -d {dataset} -s retrieval_index"
        )
        ...
        if faiss_index is None:
            if retriever_type == RETRIEVER_VECTOR_ONLY:
                raise RuntimeError(
                    ...
                    f"Run: python source/cli/run_pipeline.py -d <dataset> -s retrieval_index --rag-type vector"
                )
        ...
        if bm25_retriever is None:
            if retriever_type == RETRIEVER_BM25_ONLY:
                raise RuntimeError(
                    ...
                    f"Run: python source/cli/run_pipeline.py -d <dataset> -s retrieval_index --rag-type bm25"
                )
