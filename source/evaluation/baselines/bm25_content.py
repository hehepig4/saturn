...
# Cache rows-DB connection and the set of available tables to avoid the
# O(N) ``table_names`` lookup repeating for every table (critical for
# datasets with tens of thousands of tables such as openwikitable).
_ROWS_DB_CACHE: Dict[str, Tuple[Any, set]] = {}


def _get_rows_db_cached(dataset_name: str):
    if dataset_name in _ROWS_DB_CACHE:
        return _ROWS_DB_CACHE[dataset_name]
    rows_db_path = get_rows_db_path(dataset_name)
    if not rows_db_path.exists():
        _ROWS_DB_CACHE[dataset_name] = (None, set())
        return _ROWS_DB_CACHE[dataset_name]
    db = lancedb.connect(str(rows_db_path))
    available = set(db.table_names(limit=200000))
    _ROWS_DB_CACHE[dataset_name] = (db, available)
    return _ROWS_DB_CACHE[dataset_name]


def load_table_rows(dataset_name: str, table_id: str) -> List[List[str]]:
    """Load all rows for a specific table from rows LanceDB."""
    db, available_tables = _get_rows_db_cached(dataset_name)
    if db is None:
        logger.warning(f"Rows DB not found for dataset: {dataset_name}")
        return []
