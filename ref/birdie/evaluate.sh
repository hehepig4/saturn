...
BIRDIE_DIR="$PROJECT_ROOT/birdie"
...
NUM_QUERIES=20
EPOCHS=30
MAX_STEPS=8000  # Paper: 8000 training steps
...
SKIP_TRAINING=false
# Optional cap on Query Generator training queries (empty = use all)
LORA_MAX_SAMPLES=""
...
# OpenAI-compatible endpoint for the off-the-shelf query generator
VLLM_BASE_URL="${SATURN_LLM_BASE_URL:-http://localhost:8000/v1}"
VLLM_API_KEY="${SATURN_LLM_API_KEY:-EMPTY}"
...
TLLAMA_LORA_PATH=""
# Optionally launch a dedicated vLLM process for TLlama+LoRA.
USE_TLLAMA_VLLM=false
TLLAMA_VLLM_PORT=8001
TLLAMA_VLLM_PID=""
VLLM_PYTHON="${BIRDIE_VLLM_PYTHON:-python}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --max-steps N          Training steps; overrides epochs (default: ${MAX_STEPS})"
            ...
            echo "  --vllm-api-key KEY      Endpoint key (default: SATURN_LLM_API_KEY)"
            ...
            echo ""
            echo "TLlama options:"
            echo "  --use-tllama           Use TLlama+LoRA served at the configured endpoint"
            ...
            echo "  --use-tllama-vllm      Launch a dedicated TLlama+LoRA server"
            echo "  --tllama-vllm-port N   Port for local TLlama vLLM server (default: 8001)"
            echo "  --lora-max-samples N   Cap queries used for LoRA training (default: all)"
            echo "  BIRDIE_VLLM_PYTHON     Python executable with vLLM installed"
            echo ""
            exit 0
            ;;
        *)
...
# Keep query-generator variants from reusing each other's training artifacts.
GENERATOR_VARIANT=off_the_shelf
if [ "$USE_TLLAMA" = true ]; then
    GENERATOR_VARIANT=tllama_lora
fi
DATASET_DIR="${BIRDIE_DIR}/dataset/${GENERATOR_VARIANT}/${DATASET_NAME}"
TABLEID_DIR="${BIRDIE_DIR}/tableid"
QUERY_GEN_DIR="${SCRIPT_DIR}/patches"  # Use patched query_g.py
OUTPUT_DIR="${BIRDIE_DIR}/output/${GENERATOR_VARIANT}/${DATASET_NAME}"
...
start_tllama_vllm_server() {
    ...
    echo "Starting local vLLM server for TLlama+LoRA on port ${port}..."
    if ! python -c 'import socket, sys; s = socket.socket(); s.bind(("127.0.0.1", int(sys.argv[1]))); s.close()' "$port"; then
        echo "Port ${port} is in use. Choose --tllama-vllm-port or use an existing endpoint."
        return 1
    fi

    local server_cmd=("${VLLM_PYTHON}" -m vllm.entrypoints.openai.api_server
        --model "$model_path" --port "$port" --host 127.0.0.1
        --max-model-len 4096 --gpu-memory-utilization 0.85)
    if [ -n "$lora_path" ]; then
        server_cmd+=(--enable-lora --max-lora-rank 64
            --lora-modules "$(basename "$lora_path")=$lora_path")
    fi

    # A separate process group lets cleanup stop only this invocation's server.
    setsid "${server_cmd[@]}" > "${OUTPUT_DIR}/query_server.log" 2>&1 &
    TLLAMA_VLLM_PID=$!

    echo "Waiting for vLLM server to start (PID: ${TLLAMA_VLLM_PID})..."
    local max_wait=900
    local waited=0
    while [ $waited -lt $max_wait ]; do
        if curl -fsS "http://localhost:${port}/v1/models" > /dev/null 2>&1; then
            echo -e "${GREEN}vLLM server is ready on port ${port}${NC}"
            return 0
        fi
        # Check if process is still running
        if ! ps -p ${TLLAMA_VLLM_PID} > /dev/null 2>&1; then
            echo -e "${RED}vLLM server process died${NC}"
            tail -50 "${OUTPUT_DIR}/query_server.log"
            return 1
        fi
        sleep 2
        waited=$((waited + 2))
    ...
    echo -e "${RED}Failed to start vLLM server within ${max_wait} seconds${NC}"
    tail -30 "${OUTPUT_DIR}/query_server.log"
    return 1
}
...
stop_tllama_vllm_server() {
    if [ -n "${TLLAMA_VLLM_PID}" ]; then
        echo "Stopping vLLM server (PID: ${TLLAMA_VLLM_PID})..."
        kill -TERM -- "-${TLLAMA_VLLM_PID}" 2>/dev/null || true
        wait "${TLLAMA_VLLM_PID}" 2>/dev/null || true
        TLLAMA_VLLM_PID=""
    fi
}
...
elif [ -f "${EMBED_DIR}/table_data_embedding.npy" ]; then
...
if [ "$USE_TLLAMA_VLLM" = true ] && [ -n "${TLLAMA_LORA_PATH}" ]; then
    echo ""
    ...
    if [ -f "${TLLAMA_LORA_PATH}/adapter_model.safetensors" ]; then
        ...
        cd "${SCRIPT_DIR}"
        LORA_ARGS=(--model "${TLLAMA_MODEL_PATH}" --output-dir "${TLLAMA_LORA_PATH}")
        if [ -n "$UNIFIED_PATH" ]; then
            LORA_ARGS+=(--unified-path "$UNIFIED_PATH")
        fi
        if [ -n "${LORA_MAX_SAMPLES}" ]; then
            LORA_ARGS+=(--max-samples "$LORA_MAX_SAMPLES")
        fi
...
if [ "$SKIP_QUERY_GEN" = true ]; then
    ...
    QUERY_GEN_USED=true
    # Remap reused query text_id to NEW docids and convert to BIRDIE 'text' field
    python "${SCRIPT_DIR}/scripts/remap_train_query.py" \
        --train_query "${TRAIN_DATA}" \
    ...
    if [ "$USE_TLLAMA" = true ]; then
        ...
        if [ "$USE_TLLAMA_VLLM" = true ]; then
            ...
            echo "Using TLlama+LoRA served at ${VLLM_BASE_URL}"
            QUERY_OUT_PARENT="$(dirname "${DATASET_DIR}")"
            python "${QUERY_GEN_DIR}/query_g.py" \
                --dataset_name "${DATASET_NAME}" \
                --tableid_path "${DOCID_DIR}/id_map.json" \
                --table_data_path "${TABLE_JSON}" \
                --out_train_path "${QUERY_OUT_PARENT}" \
                ...
                --model_name "$(basename "${TLLAMA_LORA_PATH}")" \
        ...
        if ! curl -fsS -H "Authorization: Bearer ${VLLM_API_KEY}" "${VLLM_BASE_URL}/models" > /dev/null; then
    ...
    # query_g.py outputs with 'question' field; remap text_id to NEW docids + 'text'
...
if [ "${QUERY_GEN_USED:-false}" = true ]; then
    ...
    echo "BIRDIE requires synthetic queries generated by query_g.py"
...
if [ "$SKIP_TRAINING" = true ]; then
    ...
    # TORCH_MASTER_PORT allows running multiple concurrent jobs on one host (default keeps torchrun's 29500)
    torchrun --nproc_per_node=${NUM_GPUS} --master_port=${TORCH_MASTER_PORT:-29500} run.py \
