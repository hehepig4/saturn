...
# Optional cap on number of training queries (empty = use all).
# Used to subsample large datasets (e.g. openwikitable) down to align with others.
MAX_SAMPLES=""
SAMPLE_SEED=42
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --max-samples N        Cap number of training queries (default: all)"
            echo "  --sample-seed N        Seed for query subsampling (default: ${SAMPLE_SEED})"
            ...
            echo "  --output-dir PATH      Exact adapter output directory"
            exit 0
            ;;
        *)
...
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_BASE}/${DATASET_NAME}}"
mkdir -p "${OUTPUT_DIR}"
...
# Build optional sampling args for data preparation
SAMPLE_ARGS=""
if [ -n "$MAX_SAMPLES" ]; then
    SAMPLE_ARGS="--max_samples ${MAX_SAMPLES} --seed ${SAMPLE_SEED}"
    echo "Subsampling training queries to ${MAX_SAMPLES} (seed=${SAMPLE_SEED})"
fi
...
if [ -f "${OUTPUT_DIR}/adapter_config.json" ]; then
    ...
    echo "  bash ref/birdie/evaluate.sh --dataset ${DATASET_NAME} --use-tllama-vllm --tllama-lora ${OUTPUT_DIR}"
else
