...
Uses title/schema embeddings for shallow levels and table-content embeddings
for deeper levels. Table IDs follow the order in table_data.json.

Usage:
    python hierarchical_clustering.py \
        --embed_dir <embedding_directory> \
...
def cluster_recursion(
    ...
    """Recursive clustering function (BIRDIE dual-embedding).

    Shallow layers (indicate 0,1) cluster by emd1 (title+schema);
    deep layers (indicate >= 2) cluster by emd2 (table content).
    """
    ...
    for i in uni_clusters:
        ...
        if 0 < len(pos_lists) <= c:
            ...
            father_node.add_child(node)
            cluster_recursion(
                data_pos, new_docid, indicate + 1, kmeans, mini_kmeans, emd1, emd2, node, k, c
            )

    return


def run_clustering(embed_dir, table_data_path, output_dir, k=20, c=20, seed=7, depth=3):
    """Run hierarchical clustering using BIRDIE dual-embedding views.

    emd1 (title+schema) drives shallow docid layers (indicate 0,1);
    emd2 (table content) drives deep layers (indicate >= 2).
    """
    ts_path = os.path.join(embed_dir, 'table_title_schema_embedding.npy')
    data_path = os.path.join(embed_dir, 'table_data_embedding.npy')
    print(f"Loading title+schema embeddings from {ts_path}")
    emd1 = np.load(ts_path)
    print(f"Loading table-data embeddings from {data_path}")
    emd2 = np.load(data_path)
    print(f"Loaded emd1 shape: {emd1.shape}, emd2 shape: {emd2.shape}")
    ...
    if len(table_ids) != emd1.shape[0] or emd1.shape[0] != emd2.shape[0]:
        raise ValueError(f"Mismatch: {len(table_ids)} tables vs emd1 {emd1.shape[0]} vs emd2 {emd2.shape[0]}")

    # Normalize both embedding views
    emd1 = normalize(emd1, norm="l2")
    emd2 = normalize(emd2, norm="l2")
    ...
    new_docid = [[] for _ in range(len(emd1))]

    # Create root node (uses emd1 = title+schema view)
    root_center = np.mean(emd1, axis=0)
    root_radius = np.max(np.linalg.norm(emd1 - root_center, axis=1))
    root_cohesion = np.mean(np.linalg.norm(emd1 - root_center, axis=1))
    ...
    # Run recursive clustering with dual-embedding views
    print("Running hierarchical clustering...")
    cluster_recursion(
        np.array(range(len(emd1))),
        new_docid,
        0,
        kmeans,
        mini_kmeans,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--embed_dir", required=True, type=str,
                        help="Directory containing table_title_schema_embedding.npy and table_data_embedding.npy")
