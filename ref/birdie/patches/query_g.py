"""
Generate Birdie training queries through an OpenAI-compatible endpoint.

Uses table-row sampling and multiple completions per request. The endpoint can
serve the table-specific TLlama+LoRA generator or an off-the-shelf model.

Usage:
    python query_g.py --dataset_name "fetaqa" \
...
class TLlamaQueryGenerator:
    ...
    def __init__(self, vllm_url: str, model_name: str, timeout: int = 120, api_key: str = "EMPTY"):
...
def create_args():
    ...
    parser.add_argument('--api_key', type=str, default='EMPTY', help='API key for vLLM server')
