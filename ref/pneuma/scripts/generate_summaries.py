...
class SummaryGenerator:
    ...
    def __init__(
        ...
        # Resolve api_key from arg / env, fallback to dummy token for local servers
        resolved_key = (
            api_key
        ...
        # LLM token usage accumulators (indexing stage)
    ...
    def _record_usage(self, usage) -> None:
        """Thread-safe accumulation of LLM token usage."""
        if usage is None:
            return
    ...
    def get_llm_stats(self) -> Dict:
        ...
        return {
            "cumulative_by_caller": {"pneuma_summarizer": caller_stats},
            "cumulative_totals": {
...
def process_tables(
    ...
    # Write LLM token stats (indexing stage)
    llm_stats = generator.get_llm_stats()
    stats_file = output_dir / "llm_stats_index.json"
    with open(stats_file, 'w') as f:
        json.dump(llm_stats, f, indent=2)
    totals = llm_stats["cumulative_totals"]
    print(f"\nLLM index token stats -> {stats_file}")
...
def main():
    ...
    parser.add_argument("--api-key", default=None,
                        help="API key (default: env OPENAI_API_KEY / SGLANG_API_KEY)")
