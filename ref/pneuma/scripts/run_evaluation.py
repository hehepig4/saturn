...
class HybridRetriever:
    ...
    def __init__(
        ...
        # LLM token usage accumulators (inference / rerank stage)
    ...
    def _record_usage(self, usage) -> None:
        """Thread-safe accumulation of LLM token usage during reranking."""
        if usage is None:
            return
    ...
    def get_llm_stats(self) -> dict:
        """Return LLM token stats matching the main experiment's llm_stats schema."""
        total_input = self.llm_input_tokens
        total_output = self.llm_output_tokens
        caller_stats = {
        ...
        return {
            "cumulative_by_caller": {"pneuma_rerank": caller_stats},
            "cumulative_totals": {
...
def evaluate(
    ...
    # Attach LLM token stats (non-zero only when LLM reranking is used)
    results["llm_stats"] = hybrid_retriever.get_llm_stats()
    _llm_totals = results["llm_stats"]["cumulative_totals"]
    print(
        f"LLM rerank token stats -> requests: {_llm_totals['total_requests']}, "
        f"input: {_llm_totals['total_input_tokens']}, "
        f"output: {_llm_totals['total_output_tokens']}"
    )
...
def main():
    ...
    parser.add_argument("--openai-url", default="http://localhost:8000/v1",
